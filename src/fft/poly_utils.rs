...
pub fn multi_point_eval(polynomials: Vec<&[Scalar]>, point: &Scalar) -> Vec<Scalar> {
    ...
    let powers = powers_of(point, max_coefficients);
...
pub fn single_point_eval(polynomial: &[Scalar], point: &Scalar) -> Scalar {
    if polynomial.len() == 0 {
        return Scalar::zero();
    }

    // Compute powers of points
    let mut powers = powers_of(point, polynomial.len());

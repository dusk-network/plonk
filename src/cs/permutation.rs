...
mod benches {
    ...
    fn bench_perm_poly_fast(b: &mut Bencher, n : usize) {

        let w_l : Vec<_> = (0..n).map(|i|Fr::from(i as u32)).collect();
        let w_r : Vec<_> = (0..n).map(|i|Fr::from(i as u32)).collect();
        let w_o : Vec<_> = (0..n).map(|i|Fr::from(i as u32)).collect();

        let mut perm : Permutation<E> = Permutation::new();
        for i in 0..n {
           let a = perm.new_variable(Fr::from(i as u32));
            let b = perm.new_variable(Fr::from((i+1) as u32));
            let c = perm.new_variable(Fr::from((i+2) as u32));

            perm.add_variable_to_map(a,b,c, i as usize);
        }
        ...
        perm.compute_sigma_polynomials(n as usize, &domain);

        b.iter(|| perm.compute_fast_permutation_poly(
            &domain,
            &mut transcript,
            &mut rand::thread_rng(),
            &w_l,
    ...
    fn bench_perm_poly_slow(b: &mut Bencher, n : usize) {
        ...
        b.iter(|| perm.compute_slow_permutation_poly(
            &domain,
            &mut transcript,
            &mut rand::thread_rng(),
            w_l.clone().into_iter(),
    ...
    #[bench]
    fn bench_perm_poly_fast_2_13(b: &mut Bencher) {
        bench_perm_poly_fast(b, 2usize.pow(13));
    }
    #[bench]
    fn bench_perm_poly_fast_2_14(b: &mut Bencher) {
        bench_perm_poly_fast(b, 2usize.pow(14));
    }
    #[bench]
    fn bench_perm_poly_fast_2_15(b: &mut Bencher) {
        bench_perm_poly_fast(b, 2usize.pow(15));
    }
    #[bench]
    fn bench_perm_poly_fast_2_16(b: &mut Bencher) {
        bench_perm_poly_fast(b, 2usize.pow(16));
    }
    #[bench]
    fn bench_perm_poly_fast_2_17(b: &mut Bencher) {
        bench_perm_poly_fast(b, 2usize.pow(17));
    }
    #[bench]
    fn bench_perm_poly_fast_2_18(b: &mut Bencher) {
        bench_perm_poly_fast(b, 2usize.pow(18));
    }
    #[bench]
    fn bench_perm_poly_fast_2_19(b: &mut Bencher) {
        bench_perm_poly_fast(b, 2usize.pow(19));
    }
    #[bench]
    fn bench_perm_poly_fast_2_20(b: &mut Bencher) {
        bench_perm_poly_fast(b, 2usize.pow(20));
    }
    #[bench]
    fn bench_perm_poly_slow_2_13(b: &mut Bencher) {
        bench_perm_poly_slow(b, 2usize.pow(13));
    }
    #[bench]
    fn bench_perm_poly_slow_2_14(b: &mut Bencher) {
        bench_perm_poly_slow(b, 2usize.pow(14));
    }
    #[bench]
    fn bench_perm_poly_slow_2_15(b: &mut Bencher) {
        bench_perm_poly_slow(b, 2usize.pow(15));
    }
    #[bench]
    fn bench_perm_poly_slow_2_16(b: &mut Bencher) {
        bench_perm_poly_slow(b, 2usize.pow(16));
    }
    #[bench]
    fn bench_perm_poly_slow_2_17(b: &mut Bencher) {
        bench_perm_poly_slow(b, 2usize.pow(17));
    }
    #[bench]
    fn bench_perm_poly_slow_2_18(b: &mut Bencher) {
        bench_perm_poly_slow(b, 2usize.pow(18));
    }
    #[bench]
    fn bench_perm_poly_slow_2_19(b: &mut Bencher) {
        bench_perm_poly_slow(b, 2usize.pow(19));
    }
    #[bench]
    fn bench_perm_poly_slow_2_20(b: &mut Bencher) {
        bench_perm_poly_slow(b, 2usize.pow(20));
    }
}
...
fn plookup_random_difference(
    ...
    (1 + beta) * (gamma + w) * (gamma * (1 + beta) + t + (beta * t_next))
}
...
fn multizip_compute_plookup_product(
    ...
    let table_next: Vec<Scalar> = &table[1..n].push(table[0]);
    let sorted1 = sorted[0];
    let sorted1_next: Vec<Scalar> = &sorted1[1..n].push(sorted1[0]);
    let sorted2 = sorted[1];
    let sorted2_next: Vec<Scalar> = &sorted2[1..n].push(sorted2[0]);
    ...
    let product_argument = (roots, pointwise_polys)
        .into_par_iter()
        ...
        .map(|(pointwise_root, poinwise_polys)| {
            ...
            (
                ...
                lookup_septuple
                    ...
                    .map(|(f, t, t_n, _h1, _h1_n, _h2, _h2_n)| {
                        Permutation::plookup_table_irreducible(&f, beta, gamma)
                        * Permutation::plookup_random_difference(&t, &t_n, beta, gamma)
                    })
                    .product::<Scalar>(),
                // Denominator product
                lookup_septuple
                    .map(|(_f, _t, _t_n, h1, h1_n, h2, h2_n)| {
                        Permutation::plookup_random_difference(&h1, &h1_n, beta, gamma)
                        * Permutation::plookup_random_difference(&h2, &h2_n, beta, gamma)
                    })
                    .product::<Scalar>(),
            )
        })

...
fn gate(
    ...
    let a_bit_iter = BitIterator8::new(composer.variables[&a].to_bytes());
    let a_bits: Vec<_> = a_bit_iter.skip(256 - num_bits).collect();
    let b_bit_iter = BitIterator8::new(composer.variables[&b].to_bytes());
    ...
    composer
        .perm
        .add_variable_to_map(composer.zero_var, WireData::Left(composer.n));
    composer
        .perm
        .add_variable_to_map(composer.zero_var, WireData::Right(composer.n));
    composer
        .perm
        .add_variable_to_map(composer.zero_var, WireData::Fourth(composer.n));
    composer.w_l.push(composer.zero_var);
    composer.w_r.push(composer.zero_var);
    ...
    for i in 0..num_quads {
        ...
        assert!(right_accumulator - (prev_right_accum * Scalar::from(4u64)) < Scalar::from(4u64));
        ...
        let var_a = composer.add_input(left_accumulator);
        let var_b = composer.add_input(right_accumulator);
        ...
        let var_c = match i == num_quads {
            true => composer.zero_var,
            false => composer.add_input(prod_quad_fr),
        };
        let var_4 = composer.add_input(out_accumulator);
        ...
        composer
            .perm
            .add_variable_to_map(var_a, WireData::Left(composer.n));
        composer
            .perm
            .add_variable_to_map(var_b, WireData::Right(composer.n));
        composer
            .perm
            .add_variable_to_map(var_4, WireData::Fourth(composer.n));
        composer
            .perm
            .add_variable_to_map(var_c, WireData::Output(composer.n - 1));
        // Push the variables to it's actual wire vector storage
        composer.w_l.push(var_a);
        composer.w_r.push(var_b);
        composer.w_o.push(var_c);
        composer.w_4.push(var_4);
    ...
    composer
        .perm
        .add_variable_to_map(composer.zero_var, WireData::Output(composer.n - 1));
    composer.w_o.push(composer.zero_var);
    ...
    for _ in 0..num_quads {
        ...
        composer.q_arith.push(Scalar::zero());
        composer.q_o.push(Scalar::zero());
        composer.q_4.push(Scalar::zero());
        composer.q_range.push(Scalar::zero());
        match is_xor_gate {
            true => {
                composer.q_c.push(-Scalar::one());
                composer.q_logic.push(-Scalar::one());
            }
            false => {
                composer.q_c.push(Scalar::one());
                composer.q_logic.push(Scalar::one());
    ...
    composer.public_inputs.extend(zeros.iter());
    ...
    assert_eq!(
        composer.variables[&a],
        composer.variables[&composer.w_l[composer.n - 1]]
    );
    assert_eq!(
        composer.variables[&b],
        composer.variables[&composer.w_r[composer.n - 1]]
    );
    ...
    // `w_4` is holding.
    composer.w_4[composer.w_4.len() - 1]
}
...
pub fn xor_gate(
    ...
    gate(composer, a, b, num_bits, true)
}
...
pub fn and_gate(
    ...
    gate(composer, a, b, num_bits, false)
}
    ...
    fn test_logic_xor_and_constraint() {
        ...
        let res = gadget_tester(
            |composer| {
                ...
                let xor_res = xor_gate(composer, witness_a, witness_b, 10);
        ...
        let res = gadget_tester(
            |composer| {
                ...
                let xor_res = and_gate(composer, witness_a, witness_b, 10);
    ...
    fn test_logical_gate_odd_bit_num() {
        ...
        let _ = gadget_tester(
            |composer| {
                ...
                let xor_res = xor_gate(composer, witness_a, witness_b, 9);
                // Check that the XOR result is indeed what we are expecting.

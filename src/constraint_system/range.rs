...
pub fn gate(composer: &mut StandardComposer, witness: Variable, num_bits: usize) {
    ...
    let value = composer.variables[&witness];
    ...
    for i in 0..pad {
        add_wire(composer, i, composer.zero_var);
    }
    ...
    for i in pad..=num_quads {
        ...
        let accumulator_var = composer.add_input(accumulator);
        accumulators.push(accumulator_var);

        add_wire(composer, i, accumulator_var);
    }
    ...
    composer.q_m.extend(zeros.iter());
    composer.q_l.extend(zeros.iter());
    composer.q_r.extend(zeros.iter());
    composer.q_o.extend(zeros.iter());
    composer.q_c.extend(zeros.iter());
    composer.q_arith.extend(zeros.iter());
    composer.q_4.extend(zeros.iter());
    composer.q_range.extend(ones.iter());
    composer.q_logic.extend(zeros.iter());
    composer.public_inputs.extend(zeros.iter());
    composer.n += used_gates;
    ...
    *composer.q_range.last_mut().unwrap() = Scalar::zero();
    ...
    composer.assert_equal(accumulators[last_accumulator], witness);

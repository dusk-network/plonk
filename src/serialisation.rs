...
pub fn read_n(n: usize, bytes: &[u8]) -> (&[u8], &[u8]) {
    assert!(bytes.len() >= n);
...
/// Returns the remaining bytes
pub fn read_g1_affine(bytes: &[u8]) -> (G1Affine, &[u8]) {
    let (bytes48, rest) = read_n(48, bytes);
    let mut arr48 = [0u8; 48];
    arr48.copy_from_slice(bytes48);
    let g1 = G1Affine::from_compressed(&arr48).unwrap();
    (g1, rest)
}
/// Reads 48 bytes and converts it to a Commitment
/// Returns the remaining bytes
pub fn read_commitment(bytes: &[u8]) -> (Commitment, &[u8]) {
    let (g1, rest) = read_g1_affine(bytes);
    (Commitment::from_affine(g1), rest)
}
...
/// Returns the remaining bytes
pub fn read_g2_affine(bytes: &[u8]) -> (G2Affine, &[u8]) {
    let (bytes96, rest) = read_n(96, bytes);
    let mut arr96 = [0u8; 96];
    arr96.copy_from_slice(bytes96);
    let g2 = G2Affine::from_compressed(&arr96).unwrap();
    (g2, rest)
}
...
/// Returns the remaining bytes
pub fn read_scalars(bytes: &[u8]) -> (Vec<Scalar>, &[u8]) {
...
/// Returns the remaining bytes
pub fn read_polynomial(bytes: &[u8]) -> (Polynomial, &[u8]) {
    let (poly_vec, rest) = read_scalars(bytes);
    (Polynomial::from_coefficients_vec(poly_vec), rest)
}
...
/// Returns the remaining bytes
pub fn read_evaluations(domain: EvaluationDomain, bytes: &[u8]) -> (Evaluations, &[u8]) {
...
mod test {
    ...
    fn test_read_write_scalar() {
        ...
        let (got, rest) = read_scalar(&bytes);
    ...
    fn test_read_write_scalars_polynomial_evaluations() {
        ...
        let (got_scalars, rest) = read_scalars(&bytes);
        let (got_polynomial, rest) = read_polynomial(&rest);
        let (got_evaluations, rest) = read_evaluations(domain, &rest);
        assert_eq!(rest.len(), 0);
    ...
    fn test_read_write_u64() {
        ...
        for x in rand_u64s.iter() {
            let (got, rest) = read_u64(&remaining);
            assert_eq!(got, *x);
            remaining = rest;
        }
    ...
    fn test_read_write_point_comm() {
        ...
        let (got_g1_affine, rest) = read_g1_affine(&bytes);
        let (got_comm, rest) = read_commitment(&rest);
        let (got_g2_affine, rest) = read_g2_affine(&rest);
        assert_eq!(rest.len(), 0);

...
pub(crate) struct SelectorPolynomials {
    ...
    q_range: Polynomial,
    q_logic: Polynomial,
    q_fixed_base: Polynomial,
    q_curve_add: Polynomial,
...
impl StandardComposer {
    ...
    fn pad(&mut self, diff: usize) {
        ...
        self.q_fixed_base.extend(zeroes_scalar.iter());
        self.q_curve_add.extend(zeroes_scalar.iter());
    ...
    fn check_poly_same_len(&self) -> Result<(), PreProcessingError> {
        ...
        if self.q_o.len() == k
            ...
            && self.q_range.len() == k
            && self.q_logic.len() == k
            && self.q_fixed_base.len() == k
            && self.q_curve_add.len() == k
            && self.w_l.len() == k
            && self.w_r.len() == k
            && self.w_o.len() == k
        {
    ...
    pub fn preprocess_prover(
        ...
        let q_fixed_base_eval_4n = Evaluations::from_vec_and_domain(
            domain_4n.coset_fft(&selectors.q_fixed_base),
            domain_4n,
        );
        let q_curve_add_eval_4n = Evaluations::from_vec_and_domain(
            domain_4n.coset_fft(&selectors.q_curve_add),
            domain_4n,
        );
        ...
        let ecc_prover_key = widget::ecc::scalar_mul::fixed_base::ProverKey {
            ...
            q_fixed_base: (selectors.q_fixed_base, q_fixed_base_eval_4n),
        };
        ...
        let curve_addition_prover_key = widget::ecc::curve_addition::ProverKey {
            q_curve_add: (selectors.q_curve_add, q_curve_add_eval_4n),
        };
    ...
    fn preprocess_shared(
        ...
        let q_fixed_base_poly =
            Polynomial::from_coefficients_slice(&domain.ifft(&self.q_fixed_base));
        let q_curve_add_poly = Polynomial::from_coefficients_slice(&domain.ifft(&self.q_curve_add));
        ...
        let q_fixed_base_poly_commit = commit_key.commit(&q_fixed_base_poly).unwrap_or_default();
        let q_curve_add_poly_commit = commit_key.commit(&q_curve_add_poly).unwrap_or_default();
        ...
        let selectors = SelectorPolynomials {
            ...
            q_fixed_base: q_fixed_base_poly,
            q_curve_add: q_curve_add_poly,
...
mod test {
    ...
    fn test_pad() {
        ...
        assert!(composer.q_fixed_base.len() == size);
        assert!(composer.q_curve_add.len() == size);
